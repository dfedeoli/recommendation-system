...
edx %>% summarize(n_userIds = n_distinct(userId), n_movieIds = n_distinct(movieId),
  ...
  kbl(booktabs = TRUE, caption = "Distinct values for userId, movieId, titles and genres") %>% kable_styling(latex_options = c("striped", "hold_position"))
```
...
From the `r edx %>% summarize(n_distinct(movieId)) %>% .[1,1] -1` movies, we can see that only `r edx %>% group_by(movieId) %>% summarize(avg=mean(rating)) %>% filter(avg< 1.5) %>% dim() %>% .[1]` of them average below 1.5, and `r edx %>% group_by(movieId) %>% summarize(avg=mean(rating)) %>% filter(avg> 4.25) %>% dim() %>% .[1]` films average over 4.25 rating.
...
Now that we have our training and test sets, we can start evaluating prediction models. In this report, we will begin by following the book by Prof. Irrizarry, with the Naive RMSE, Movie Effect, Movie + User Effect and Regularized Movie + User Effects, but we will also apply a method that includes the Genre Effect and a Matrix Factorization one. And to evaluate their quality, we will use de RMSE function.
