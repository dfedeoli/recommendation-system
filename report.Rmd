...
Companies like _Amazon_, _YouTube_ or _Netflix_ possess massive user ratings datasets, which they were able to collect from thousands of products sold to or used by customers. Recommendation systems use those ratings to make user-specific recommendations. With regard to movies, we can predict what rating a user  would give movie pictures they haven't seen by analyzing the ones he or she likes to watch, and then recommend those with relatively high predicted ratings. These are more complicated machine learning challenges because each outcome has a different set of predictors, seeing that different users rate different movies and different quantities of movies.
...
We first calculate various sets of predictions and their respective RMSE values by testing $\lambda$ varying from 0 to 10, with a 0.25 step-size, and then determine which regularization parameter gives us the lowest RMSE.
...
```
 ...
 From tuning, we obtained the optimal parameters `dim` = `r optimal_params[[1]]`, `costp_l1` = `r optimal_params[[2]]`, `costp_l2` = `r optimal_params[[3]]`, `costq_l1` = `r optimal_params[[4]]`, `costql2`=`r optimal_params[[5]]`, `lrate` = `r optimal_params[[6]]`, `loss_fun` = `r optimal_params[[7]]`. The model was then trained with these values, and it is now possible to predict ratings for the test set with `r$predict()`.
...
where we perceive `r sum(predicted_ratings<0.5)` values under 0.5 rating and `r sum(predicted_ratings>5)` over 5.
